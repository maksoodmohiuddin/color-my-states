...
.controller('ColorCtrl', function($scope, $ionicLoading, $http, $state, $ionicPopover, $ionicPopup, States, GeoLocation, SessionService) {
  ...
  canvas.setBackgroundImage('img/south_carolina.gif', canvas.renderAll.bind(canvas), {
     height: 680,
     width: 680
   });
  ...
  $scope.GetGeoLocationState = function () {
    ...
    $http.get(googleMapsReverseGeocodingAPI)
      .success(function (response) {
        alert(response.results[0].address_components);
        alert(response.results[0].address_components[0]);
...
.controller('AboutCtrl', function($scope, $ionicPopover, $ionicPopup) {
